...
export function VoicePickerStandalone({ value, onChange }: Props) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState("");
  const [tab, setTab] = useState<"gemini" | "elevenlabs">(
    value.voiceSource === "elevenlabs" ? "elevenlabs" : "gemini"
  );
  ...
  const fetchVoices = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setVoicesError("Could not load ElevenLabs voices");
      }
    } catch {
  ...
  const selectGeminiVoice = (name: string) => {
    onChange({ voiceId: name, voiceSource: "gemini", voiceName: name });
    setOpen(false);
  };
  ...
  const displayName = value.voiceName || value.voiceId || "Select voice";
  const sourceLabel = value.voiceSource === "elevenlabs" ? "ElevenLabs" : "Gemini";

  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button className="flex items-center gap-2 bg-background border border-border rounded px-2 py-1.5 text-sm hover:border-muted-foreground transition-colors min-w-[160px] w-full">
          <Volume2 size={12} className="text-purple-400 shrink-0" />
          <span className="truncate flex-1 text-left">{displayName}</span>
          {value.voiceId && (
            <span
              className={`text-[9px] px-1 py-0.5 rounded shrink-0 ${
                value.voiceSource === "elevenlabs"
      ...
      <PopoverContent align="start" className="w-80 p-0 max-h-[500px] flex flex-col">
        {/* Tabs */}
        ...
        <div className="overflow-y-auto flex-1 max-h-[300px]">
              ...
              {filteredGemini.map((v) => (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 rounded text-xs text-left hover:bg-muted/50 transition-colors ${
                    value.voiceId === v && value.voiceSource === "gemini"
          ...
          {tab === "elevenlabs" && !showCreate && (
            <div className="p-1">
              {loadingVoices ? (
                ...
                <p className="text-[10px] text-red-400 text-center py-4">{voicesError}</p>
              ) : (
                <>
                  {filteredEl.map((v) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-2 px-3 py-2 rounded text-xs text-left hover:bg-muted/50 transition-colors ${
                        value.voiceId === v.voice_id && value.voiceSource === "elevenlabs"
                    ...
                    >
                      {v.preview_url && (
                        ...
                        >
                          {playingUrl === v.preview_url ? <Pause size={10} /> : <Play size={10} />}
                        </button>
                      )}
                      ...
                      <span className="text-[9px] text-violet-400 shrink-0">11Labs</span>
                    </button>
                  ))}
                  {filteredEl.length === 0 && !loadingVoices && (
                    <p className="text-[10px] text-muted-foreground text-center py-4">
                      {search ? "No matching voices" : "No ElevenLabs voices found"}
                    </p>
                  )}
                </>
              )}
          ...
          {tab === "elevenlabs" && showCreate && (
            <div className="p-3 space-y-3">
              <div>
                <label className="text-[10px] text-muted-foreground font-medium">Voice Name</label>
                <input
                  value={newVoiceName}
              ...
              <div>
                <label className="text-[10px] text-muted-foreground font-medium">Voice Description (prompt)</label>
                <textarea
                  value={newVoiceDesc}
                  onChange={(e) => setNewVoiceDesc(e.target.value)}
                  placeholder="A deep, warm male voice with a confident tone..."
              ...
              <button
                ...
                disabled={creating || !newVoiceName.trim() || !newVoiceText.trim()}
              ...
              {previews.length > 0 && (
                <div className="space-y-2 pt-1">
                  <p className="text-[10px] text-muted-foreground font-medium">Pick a preview to save:</p>
                  {previews.map((p, i) => (
                    <div key={p.generated_voice_id} className="flex items-center gap-2 bg-muted/30 rounded p-2">
                      <button
                        onClick={() => playBase64(p.audio_base_64)}
                        className="shrink-0 p-1 hover:text-violet-400"
                      >
                        {playingUrl === `data:audio/mpeg;base64,${p.audio_base_64}` ? (
                          <Pause size={12} />
                        ) : (
                          <Play size={12} />
                        )}
        ...
        {tab === "elevenlabs" && (
          <div className="border-t border-border p-2">
            <button
              onClick={() => { setShowCreate(!showCreate); setPreviews([]); }}
